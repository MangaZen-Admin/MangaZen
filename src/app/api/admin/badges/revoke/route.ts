...
export async function DELETE(request: Request) {
  ...
  const body = (await request.json()) as RevokeBadgeBody;
  ...
  const [user, badge] = await Promise.all([
    prisma.user.findUnique({
      ...
      select: {
        id: true,
        badges: { where: { id: body.badgeId }, select: { id: true } },
      },
    }),
  ...
  if (user.badges.length === 0) {
    return NextResponse.json({ error: "BADGE_NOT_ASSIGNED" }, { status: 409 });
  }
